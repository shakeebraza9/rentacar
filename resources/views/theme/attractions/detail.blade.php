            ...
            <div class="row mb-5 mt-4">
                ...
                <div class="col-md-6">
                    <div id="slider-hero-experience-details" class="carousel slide h-100" data-bs-ride="carousel">
                        ...
                        <button class="carousel-control-prev" type="button" data-bs-target="#slider-hero-experience-details" data-bs-slide="prev">
                            <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                            <span class="visually-hidden">Previous</span>
                        </button>
                        <button class="carousel-control-next" type="button" data-bs-target="#slider-hero-experience-details" data-bs-slide="next">
            ...
            <div class="card mb-3 mt-3">
                <div class="card-header bg-light">
                    <div class="row mt-2">
                        <div class="col">
                            <h3>{{ $ticket->title }}</h3>
                            <h6 class="text-primary btn-detail" style="cursor:pointer;" data-id="{{ $ticket->id }}">View Details</h6>
                        </div>
                        <div class="col-auto">
                            <h5 id="individual-slash-price-{{ $ticket->id }}">
                                @if ($ticket->discount_price)
                                    <p class="m-0 text-muted"><del>RM {{ number_format($ticket->selling_price, 2) }}</del></p>
                                @endif
                            </h5>
                        ...
                        <div class="col-md-3 border-end mb-2">
                            <ul class="nav flex-column nav-pills" id="pills-tab-{{ $ticket->id }}" role="tablist">
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link link-dark active" id="ticket-tab-{{ $ticket->id }}" data-bs-toggle="pill"
                                        data-bs-target="#ticket-{{ $ticket->id }}" type="button" role="tab"
                                        aria-controls="ticket-{{ $ticket->id }}" aria-selected="true">Ticket Details</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link link-dark" id="validity-tab-{{ $ticket->id }}" data-bs-toggle="pill"
                                        data-bs-target="#validity-{{ $ticket->id }}" type="button" role="tab"
                                        aria-controls="validity-{{ $ticket->id }}" aria-selected="false">Validity
                                        Information</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link link-dark" id="reservation-tab-{{ $ticket->id }}" data-bs-toggle="pill"
                                        data-bs-target="#reservation-{{ $ticket->id }}" type="button" role="tab"
                                        aria-controls="reservation-{{ $ticket->id }}" aria-selected="false">Reservation
                                        Information</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link link-dark" id="cancellation-tab-{{ $ticket->id }}" data-bs-toggle="pill"
                                        data-bs-target="#cancellation-{{ $ticket->id }}" type="button" role="tab"
                                        aria-controls="cancellation-{{ $ticket->id }}" aria-selected="false">Cancellation
                                        Policy</button>
                                </li>
                                <li class="nav-item" role="presentation">
                                    <button class="nav-link link-dark" id="redemption-tab-{{ $ticket->id }}" data-bs-toggle="pill"
                                        data-bs-target="#redemption-{{ $ticket->id }}" type="button" role="tab"
                                        aria-controls="redemption-{{ $ticket->id }}" aria-selected="false">Redemption
                                        Method</button>
                                </li>
                            </ul>
                        </div>
                        <div class="col-md mt-4 mt-md-0">
                            <div class="tab-content" id="pills-tabContent-{{ $ticket->id }}">
                                ...
                                <div class="tab-pane fade" id="reservation-{{ $ticket->id }}" role="tabpanel"
                                    ...
                                    <ul>
                                        <li>Reservation details: {{ $ticket->reservation_info ?? 'Please contact us for more details.' }}</li>
                                    </ul>
                                </div>
                                <div class="tab-pane fade" id="cancellation-{{ $ticket->id }}" role="tabpanel"
                                    aria-labelledby="cancellation-tab-{{ $ticket->id }}">
                                    <ul>
                                        <li>{{ $ticket->cancellation_policy ?? 'No cancellation policy available.' }}</li>
                                    </ul>
                                </div>
                                <div class="tab-pane fade" id="redemption-{{ $ticket->id }}" role="tabpanel"
                                    aria-labelledby="redemption-tab-{{ $ticket->id }}">
                                    <ul>
                                        <li>{{ $ticket->redemption_info ?? 'No redemption information available.' }}</li>
                                    </ul>
                                </div>
                            </div>
                        </div>
