...
@section('content')
    <div class="row page-titles">
        <div class="col-md-5 align-self-center">
            <h4 class="text-themecolor">Create  Blog
            </h4>
        </div>
    ...
    <div class="row">
        <div class="col-lg-12">
            <form method="post" action="{{ URL::to('admin/blog/store') }}">
                @csrf
                <div class="row">
                    <div class="col-md-12">
                        <section class="card">
                            ...
                            <div class="card-body">
                                ...
                                <div class="form-group">
                                    ...
                                    <input required type="text" value="{{ old('slug') }}" name="slug" class="form-control slug" placeholder="Slug">
                                    @error('slug')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
                                ...
                                <div class="form-group my-2">
                                    ...
                                    <select name="image_id" class="form-control select2" id="image-selector">
                                        ...
                                        @foreach($filemanager as $file)
                                            <option value="{{ $file->id }}" data-image="{{ asset($file->path) }}"
                                                @if(old('image_id') == $file->id) selected @endif>
                                                {{ $file->title }}
                                            </option>
                                        @endforeach
                                    </select>
                                    <div class="selected-image-preview mt-2">
                                        @if(old('image_id'))
                                            <img class="pt-3" id="preview-img" style="width: 100px; height: 100px;" src="{{ asset($filemanager->where('id', old('image_id'))->first()->path ?? '') }}" alt="Thumbnail" />
                                        @else
                                ...
                                <div class="form-group">
                                    ...
                                    <textarea name="description" class="form-control" rows="4" placeholder="Enter detailed description">{{ old('description') }}</textarea>
                                    @error('description')
                                        <p class="text-danger">{{ $message }}</p>
                                    @enderror
                                </div>
